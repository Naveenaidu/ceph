    ...
    const std::vector<std::pair<std::string_view,std::string_view>> &labels) const
{
  ...
  */
  ...
  if (!labels.empty()){
    ...
    resolved_path = counter_name_with_labels + "." + sub_counter_name
  } else {
    resolved_path = counter_name + "." + sub_counter_name;
  }
    ...
    const std::string &svc_id)
{
  ...
  auto dump_sub_counter_information = [] (PyFormatter *f, PerfCounterType type){
    f->open_object_section(type.counter_name);
  ...
  if (!daemons.empty()) {
    for (auto& [key, state] : daemons) {
      ...
      with_gil(no_gil, [&, key=ceph::to_string(key), state=state] {
        ...
        f.open_object_section(key.c_str()); // Main Object Section
